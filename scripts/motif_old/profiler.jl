...
function run_extract_motifs()
    if length(ARGS) != 4
        println(stderr, "Usage: julia profiler.jl <input.tsv.gz> <output_file> <motif_length> <reference.fasta>")
        exit(1)
    end
...
@profilehtml run_extract_motifs()

println("Profiling completed. Results have been saved to 'profile.html'.")
println("You can transfer this file to a machine with a web browser to view the detailed results.")