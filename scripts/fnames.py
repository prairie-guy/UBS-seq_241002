...
def fname(path, stem, suffix, extra=''):
    ...
    if not suffix: return Path(path)/f"{stem}" if not extra else Path(path)/f"{stem}_{extra}"
...
class ListDict(dict):
    """
    ListDict is a Specialized Dict that returns a list of values, but can be indexed by key

    Example:
    paths = ListDict({'pUC19':'path_map_pUC19', 'lambda': 'path_map_lambda'})
    paths -> ['path_map_pUC19', 'path_map_lambda']
    paths['pUC19'] -> 'path_map_pUC19'
    """
    def __iter__(self):
    ...
    def __add__(self, other):
        ...
        # Create a new ListDict by combining the keys and values of both ListDicts
        combined_dict = ListDict(self)
        combined_dict.update(other)
        return combined_dict
    def __getitem__(self, key):
        # If key is an integer, treat it as an index
        if isinstance(key, int):
            # Convert to list and return the item at the specified index
            return list(self.values())[key]
        # Otherwise, treat it as a normal dictionary key
        return super().__getitem__(key)

def mkpaths(path, refs):
    """
    mkpaths2 :: str -> [str] -> {ref: IO_Path}
    ListDict

    Creates and returns a ListDict of multiple (not over-writting) new dir `path_ref`, one for each ref

    Expample:
    mkpaths('map', ['pUC19','lambda']) -> ListDict({'pUC19': map_pCU19/,'lambda' map_lamba/})
    """
    return ListDict({ref: mkpath(f'{path}_{ref}') for ref in refs})


def fnames(dir_or_files, suffix="", key='full'):
    """
